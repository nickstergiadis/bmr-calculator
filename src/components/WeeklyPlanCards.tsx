    ...
    <div className="grid gap-4 lg:grid-cols-3">
      {weeklyStructures.map((plan) => (
        <article key={plan.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
          <p className="text-xs font-semibold uppercase tracking-wide text-clinical-700">{plan.shortLabel}</p>
          <h3 className="mt-2 text-base font-semibold text-slate-900">{plan.title}</h3>
